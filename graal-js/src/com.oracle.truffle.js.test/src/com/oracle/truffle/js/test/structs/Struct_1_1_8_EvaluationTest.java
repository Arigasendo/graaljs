...
/**
 * Tests related to section 1.1.8 Runtime Semantics: Evaluation.
 */
public class Struct_1_1_8_EvaluationTest {

    /**
     * Tests steps 1-2 of StructDeclaration evaluation.
     * - BindingStructDeclarationEvaluation is performed
     * - Returns empty (undefined)
     */
    @Test
    public void testStructDeclarationEvaluation() {
        try (Context ctx = JSTest.newContextBuilder().option(ECMASCRIPT_VERSION_NAME, ECMASCRIPT_VERSION_STAGING).build()) {
            org.graalvm.polyglot.Source source = org.graalvm.polyglot.Source.newBuilder("js", """
                // Test struct declaration in statement context
                struct Point { x = 0 }

                // Verify Point was declared
                if (typeof Point !== 'function') {
                    throw new Error('Point should be declared');
                }
                """, "test").buildLiteral();
            ctx.eval(source);
        }
